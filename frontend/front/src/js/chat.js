...
async function createNewSession() {
  try {
    const newSession = await createSession(); // api.js 호출
...
async function addMessage(role, text) {
  ...
  if (role === "user") {
    try {
      // 1. 서버에 사용자 메시지 전송 (백엔드가 FastAPI 호출 후 봇 응답을 반환)
      // (api.js의 saveMessage가 botResponse 객체를 반환한다고 가정)
      const botResponse = await saveMessage(sess.id, messageData);
